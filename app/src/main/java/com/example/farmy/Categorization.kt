...
class Categorization(assetManager: AssetManager,modelPath: String,labelPath: String, inputSize: Int) {
    ...
    fun recognizeImage(bitmap: Bitmap): List<com.example.farmy.Categorization.Categorization>{
    ...
    private fun getSortedResult(result: Array<FloatArray>): List<com.example.farmy.Categorization.Categorization> {
        val pq = PriorityQueue(
            GREAT_OUTCOME_MXX,
            kotlin.Comparator<com.example.farmy.Categorization.Categorization>{
        ...
        for (i in ROW_LINE.indices){
            ...
            if(confidence >= POINT_THRHLD){
                pq.add(com.example.farmy.Categorization.Categorization(
                    ""+i,
                    if(ROW_LINE.size > i) ROW_LINE[i] else "Unknown", confidence
                ))
            }
        }
        val recognition = ArrayList<com.example.farmy.Categorization.Categorization>()
